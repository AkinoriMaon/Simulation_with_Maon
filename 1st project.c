...
#include <string.h> // 문자열 길이를 알 수 있는 strlen 함수를 위해 추가한 헤더파일
...
typedef struct _Ranking // 최고 점수를 담기 위해서 만든 구조체
{
	char nickname[20]; // 이름
	int score; // 점수
}Rank;
...
int main()
{
	...
	int i = 0, exit, like = 0, lenght, score; // i는 반복문을 위해서, exit은 반환값, like는 호감도, lenght는 이름 길이, score는 점수를 담기 위한 변수이다.
	...
	gotoxy(22, 6);
	puts("공백 시 공백 전 만 닉네임으로 설정함. Ex) Tes t ==> Tes만 닉네임으로 설정"); // scanf_s 함수의 한계
	gotoxy(14, 8);
	puts("공백을 원하는 경우, 공백을 원하는 자리에 _를 입력해주세요. 단, 닉네임을 _로만 입력하지 마세요."); // 공백을 넣고 싶은 사람을 위해서 배려
	...
	while (1)
	{
		...
		gotoxy(55, y);
		scanf_s("%s", nickname, 20);
		
		lenght = strlen(nickname); // 문자열 길이를 구해서 조건을 무시했는지 검사하기 위해서 사용하였다.
		
		if (lenght == 0) // 범위를 초과시킨 후 길이 값을 출력해보았더니 0이 나와서 조건식을 이렇게 줘보았다.
		{
			y++;
			gotoxy(30, y);
			puts("이름이 범위를 초과하였습니다. 다시 입력해주세요.");
			while (getchar() != '\n') {} // 버퍼 초기화
		}
		else
		{
	...
	while (getchar() != '\n') {} // 이름을 띄어쓰기로 입력하였을 경우, 뒤에 이어지는 것은 화이트 스페이스를 기준으로 끝내는 %s의 특성상 버퍼에 남게 되기 때문에 여기서 제거해준다.
	i = 0;
	while (nickname[i] != '\0') // 공백을 넣고 싶은 경우 _를 입력하였을 경우, 해당 자리에 공백 발생
	{
		if (nickname[i] == '_')
		{
			nickname[i] = ' ';
